...
def hill_climbing(num_customers, is_weekend, store_name):
    best_j, best_x, best_pr = 0, 1, 0.0
    best_profit, _ = calculate_profit(num_customers, best_j, best_x, best_pr, is_weekend, STORE_PARAMS[store_name])
    ...
    for _ in range(100):
        ...
        for dj, dx, dpr in moves:
            ...
            p, _ = calculate_profit(num_customers, nj, nx, npr, is_weekend, STORE_PARAMS[store_name])
            if p > best_profit:
                best_profit = p
...
def generate_plan():
    features = ['Day_of_Week', 'Is_Weekend', 'Month', 'Quarter', 'Day_of_Year',
                'Is_Black_Friday', 'Is_Tourist_Event', 'Is_Holiday', 'Is_Memorial_Day',
                'Event_Nearby', 'Holiday_Nearby', 'Lag_Customers_7', 'Lag_Customers_14', 
    ...
    for s in STORE_PARAMS.keys():
        ...
        for h in range(1, 8):
            ...
            j, x, pr, profit = hill_climbing(num_c, row['Is_Weekend']==1, s)
            results.append([s, row['Date'], int(num_c), j, x, pr, profit])
            
    return pd.DataFrame(results, columns=['Loja', 'Data', 'Pred_Clientes', 'J', 'X', 'PR', 'Lucro'])

if __name__ == "__main__":
    final_plan = generate_plan()
    print("\n--- PLANO SEMANAL OTIMIZADO ---")
    print(final_plan.to_string(index=False))
    final_plan.to_csv('reports/weekly_plan.csv', index=False)